...
class CrawlBSF:
    ...
    def enqueueUrl(self, url):
        if url not in self.bloom_url_queue and hashlib.md5(url.encode('utf8')).hexdigest() not in crawler.bloom_downloaded_urls:
...
def get_page_content(cur_url):
    ...
    dumd5 = hashlib.md5(cur_url.encode('utf8')).hexdigest()
